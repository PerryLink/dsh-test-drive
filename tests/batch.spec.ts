/**
 * The batch producer over the REAL jobs registry: serial drives append
 * progress to the job's output ring, settlement writes the matrix and the
...
describe('startBatchJob through the real registry', () => {
  it('streams progress, settles completed, and names the matrix', async () => {
    ...
    const snapshot = await harness.ctx.jobs.wait(JobId(String(jobId)), 10_000, harness.agent.id)
    ...
    const output = harness.ctx.jobs.read(JobId(String(jobId)), harness.agent.id)
  ...
  it('kills cleanly when cancelled', async () => {
    ...
    const kill = harness.ctx.jobs.kill(JobId(String(jobId)), harness.agent.id, 'test cancel')
