...
- `tests/` — vitest; real Cordis `Context` + real `SessionStore`/`Session`/`ToolRuntime`/`AgentRegistry`/`LocalJobRegistry`/`Storage`+`DomainFacility` from the pinned `0.1.7-rc.1` peers; the subprocess provider is a scripted subclass of the REAL `SubprocessRuntime` (or the real `LocalSubprocessRuntime` in `e2e.spec.ts`).
...
- `typecheck` resolves `@deepseek-ai/*` through tsconfig paths to the local harness checkout; `typecheck:ci` clears the paths and checks against the published `0.1.7-rc.1` types. Both must stay green.
